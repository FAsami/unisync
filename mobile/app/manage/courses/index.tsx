...
import { GET_ALL_COURSES } from '@/lib/graphql-operations'
...
const CoursesScreen = () => {
  ...
  const filteredData = useMemo(() => {
    if (!data?.academic_course) return []
    return data.academic_course.filter((item: any) => {
      const matchesSearch =
      ...
      >
        ...
        >
          {filteredData.map((course: any, index: number) => (
            <TouchableOpacity
              key={course.id}
              onPress={() => router.push(`/manage/courses/${course.id}`)}
                  ...
                  <VStack>
                    ...
                    {course.department && (
                      <Text className="text-xs text-typography-500">
                        {course.department.name} • {course.credit_hours} Credits
                        • {course.course_type}
                      </Text>
                    )}
                  </VStack>
