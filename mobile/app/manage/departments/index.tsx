...
import { GET_DEPARTMENTS, DELETE_DEPARTMENT } from '@/lib/graphql-operations'
...
const DepartmentsScreen = () => {
  ...
  const filteredData = data?.academic_department.filter((item: any) => {
    const matchesSearch =
    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-background-950">
      ...
      <HStack className="items-center justify-between px-4 py-3 bg-white dark:bg-background-900 border-b border-outline-100 dark:border-outline-800">
        ...
        <TouchableOpacity
          onPress={() => {
            haptics.light()
            router.push('/manage/departments/new')
          }}
      ...
      >
        ...
        >
          {filteredData?.map((dept: any, index: number) => (
            <TouchableOpacity
              key={dept.id}
              onPress={() => router.push(`/manage/departments/${dept.id}`)}
