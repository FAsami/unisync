...
import { GET_ALL_STUDENTS } from '@/lib/graphql-operations'
...
const StudentsListScreen = () => {
  ...
  const students = data?.user_profile || []
  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity
      onPress={() => {
        haptics.light()
        router.push(`/manage/students/${item.user_id}`)
      }}
      ...
      <HStack className="justify-between items-center">
        <VStack>
          ...
          <Text className="text-sm text-typography-500">
            {item.student_id} • {item.department?.code}
          </Text>
          <Text className="text-xs text-typography-400 mt-1">
            Batch: {item.batch?.name} • Section: {item.section?.name}
          </Text>
        </VStack>
    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-background-950">
      ...
      <VStack className="bg-white dark:bg-background-900 border-b border-outline-100 dark:border-outline-800 pb-2">
        <HStack className="items-center justify-between px-4 py-3">
          ...
          <TouchableOpacity
            onPress={() => router.push('/manage/students/new')}
